...
import {header, labelButton, navButton} from "../styledefs.emotion";
...
function getLabel({activeDate, view, range}) {
	if (!activeDate?.isValid || !view) {
		return "";
	}
	if (view === "day") {
		return activeDate?.toFormat?.("MMMM yyyy");
	}
	if (view === "month") {
...
export default function Header() {
	...
	return <div css={header}>
		<PreviousButton css={navButton}
		ariaLabel={previousButtonARIALabel}
		onClick={prev} />
		<LabelButton css={labelButton}
		buttonLabel={label}
		ariaLabel={labelButtonARIALabel}
		onClick={onClick} />
		<NextButton css={navButton}
